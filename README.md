...
Pesan biasa yang mengandung URL pada grup jadwal aktif akan diproses otomatis tanpa tag bot. Pemeriksaan otomatis hanya memanggil VirusTotal dan tidak memanggil Jina Reader maupun Groq. Bot memberi reaksi `🧐` saat memeriksa; jika link belum terdeteksi berbahaya, reaksinya diganti menjadi `✅` tanpa pesan teks. Jika link terdeteksi mencurigakan atau berbahaya, bot memberi reaksi `❌`, membalas pesan, menghapus pesan sumber bila memiliki izin admin, lalu mengirim penjelasan risiko. Bot memeriksa maksimal tiga URL dalam satu pesan.
